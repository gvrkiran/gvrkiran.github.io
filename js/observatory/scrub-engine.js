...
function mountScrollWorld(container, config) {
  ...
  function loadClip(s) {
    ...
    // This build intentionally keeps phones poster-only. The five landscape
    // clips are tuned for desktop scrubbing; loading them on a portrait phone
    // would add decode cost without a properly composed mobile camera chain.
    if (reduce || isMobile() || s.loading || !s.clip) return;
    s.loading = true;
  ...
  function read() {
    ...
    for (let i = 0; i < NSEG; i++) {
      ...
      // Keep later segments above earlier ones. Their opacity already ramps up
      // before the boundary, so stable ordering produces a real crossfade in
      // both scroll directions. Switching the active segment to the top at the
      // boundary turns that intended blend into a visible hard cut.
    ...
    for (let i = 0; i < N; i++) {
      ...
      const c = copies[i];
      c.style.opacity = cop;
      const drift = (0.5 - pr) * 4;
      c.style.transform = reduce ? 'none'
        : (isMobile() ? `translateY(${drift}vh)` : `translateY(calc(-50% + ${drift}vh))`);
